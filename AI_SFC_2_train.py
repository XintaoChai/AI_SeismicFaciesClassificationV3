...
from pyseismic_local.models_networks_ANNs.BridgeNet_Keras import *
...
parser.add_argument('--loss_used', default=2, type=int, help='loss_used')
parser.add_argument('--lr', default=0.000100, type=float, help='lr')
parser.add_argument('--pool_way', default=1, type=int, help='pool_way')
parser.add_argument('--act_hide', default=1, type=int, help='act_hide')
...
parser.add_argument('--use_BN', action='store_true', help='use_BN')
...
parser.add_argument('--act_last', default=0, type=int, help='act_last')
...
parser.add_argument('--conv2act_repeat', default=1, type=int, help='conv2act_repeat')
parser.add_argument('--reproduce', default=1, type=int, help='reproduce')
parser.add_argument('--dilation_rate', default=1, type=int, help='dilation_rate')
...
training_data_dir = module_data_dir + '/' + 'training_data_' + str(int(args.patch_rows)) + 'x' + str(
    int(args.patch_cols)) + '_stride' + str(args.stride[0]) + '_' + str(args.stride[1]) + '_Ychannels' + str(
    args.Y_channels)
training_data_path = training_data_dir + '/'
validation_data_dir = module_data_dir + '/' + 'validation_data_' + str(int(args.patch_rows)) + 'x' + str(
    int(args.patch_cols)) + '_Ychannels' + str(args.Y_channels)
...
################################################################################
model_folder_name = '2patchR' + str(args.patch_rows) + 'C' + str(args.patch_cols) + '_batch' + str(args.batch_size) + '_number' + str(args.training_number) + '_stride' + str(
    args.stride[0]) + '_' + str(
    args.stride[1]) + '_loss' + str(args.loss_used) + '_lr' + str(round(args.lr * 10000000)) + '_pool' + str(args.pool_way) + '_kernelSize' + str(
    args.kernel_size) + '_Net' + str(args.BridgeNet_used) + '_kernels' + str(
    args.kernels_all[0]) + '_repeat' + str(args.conv2act_repeat) + '_repro' + str(args.reproduce)
...
if __name__ == '__main__':
    ...
    validation_X = np.zeros(shape=[available_validation_number, args.patch_rows, args.patch_cols, X_channels],
                            dtype='float32')
    validation_Y = np.zeros(shape=[available_validation_number, args.patch_rows, args.patch_cols, args.Y_channels],
                            dtype='float32')
    ...
    for j in range(available_validation_number):
        ...
        validation_X_data = bin_data_reader2D_float32(validation_X_dataID, args.patch_rows, args.patch_cols, X_channels,
                                                      1)
        validation_Y_data = bin_data_reader2D_float32(validation_Y_dataID, args.patch_rows, args.patch_cols,
                                                      args.Y_channels, 1)
    ...
    if args.BridgeNet_used == 1:
        # 该处通过调用自己编写的BridgeNet_1函数构建人工神经网络
        model = BridgeNet_1(nD=args.nD, Y_channels=args.Y_channels_model, kernel_size=args.kernel_size, use_BN=args.use_BN,
                            kernels_all=args.kernels_all[0:(args.BridgeNet_used + 1)], act_hide=args.act_hide, act_last=args.act_last,
                            pool_way=args.pool_way, drop_rate=args.drop_rate, conv2act_repeat=args.conv2act_repeat, dilation_rate=args.dilation_rate)
    if args.BridgeNet_used == 2:
        # 该处通过调用自己编写的BridgeNet_2函数构建人工神经网络
        model = BridgeNet_2(nD=args.nD, Y_channels=args.Y_channels_model, kernel_size=args.kernel_size, use_BN=args.use_BN,
                            kernels_all=args.kernels_all[0:(args.BridgeNet_used + 1)], act_hide=args.act_hide, act_last=args.act_last,
                            pool_way=args.pool_way, drop_rate=args.drop_rate, conv2act_repeat=args.conv2act_repeat, dilation_rate=args.dilation_rate)
    if args.BridgeNet_used == 3:
        # 该处通过调用自己编写的BridgeNet_3函数构建人工神经网络
        model = BridgeNet_3(nD=args.nD, Y_channels=args.Y_channels_model, kernel_size=args.kernel_size, use_BN=args.use_BN,
                            kernels_all=args.kernels_all[0:(args.BridgeNet_used + 1)], act_hide=args.act_hide, act_last=args.act_last,
                            pool_way=args.pool_way, drop_rate=args.drop_rate, conv2act_repeat=args.conv2act_repeat, dilation_rate=args.dilation_rate)
    if args.BridgeNet_used == 4:
        # 该处通过调用自己编写的BridgeNet_4函数构建人工神经网络
        model = BridgeNet_4(nD=args.nD, Y_channels=args.Y_channels_model, kernel_size=args.kernel_size, use_BN=args.use_BN,
                            kernels_all=args.kernels_all[0:(args.BridgeNet_used + 1)], act_hide=args.act_hide, act_last=args.act_last,
                            pool_way=args.pool_way, drop_rate=args.drop_rate, conv2act_repeat=args.conv2act_repeat, dilation_rate=args.dilation_rate)
    if args.BridgeNet_used == 5:
        # 该处通过调用自己编写的BridgeNet_5函数构建人工神经网络
        model = BridgeNet_5(nD=args.nD, Y_channels=args.Y_channels_model, kernel_size=args.kernel_size, use_BN=args.use_BN,
                            kernels_all=args.kernels_all[0:(args.BridgeNet_used + 1)], act_hide=args.act_hide, act_last=args.act_last,
                            pool_way=args.pool_way, drop_rate=args.drop_rate, conv2act_repeat=args.conv2act_repeat, dilation_rate=args.dilation_rate)

    # kernel_size设定卷积层中卷积核的大小
    # use_BN设定是否使用BatchNormalization批归一化层
    # act_last设定最后输出层所用的激活函数
    # pool_way设定所用的池化方式
    ...
    checkpointer = ModelCheckpoint(os.path.join(model_log_dir, 'model_{epoch:03d}.hdf5'), verbose=1,
                                   period=args.save_every)
    csv_logger = CSVLogger(os.path.join(model_log_dir, 'log.csv'), append=True, separator=',')
    lr_scheduler = LearningRateScheduler(lr_scheduler)
    tensorboard = TensorBoard(log_dir=model_log_dir, histogram_freq=0, batch_size=2, write_graph=True, write_grads=True,
                              write_images=True)
